...
import (
	...
	gen "github.com/BlackCodes/micro-swagger/protoc-gen-micro-swagger/internal/generator"
	...
	if g.reg.IsAllowMerge() {
		...
		for _, file := range openapis {
			...
			if server := g.reg.GetoptionsServer(); len(server) > 0 && !strings.Contains(server, "0.0.0.0") {
				...
				bytes.NewReader(by)
				rsp, err := http.Post(uri, "application/json", bytes.NewReader(by))
				if err != nil {
					glog.Errorf("send data to server err:%v", err)
				}
				if rsp.StatusCode == 200 {
					glog.V(1).Info("send data to server success")
				}
			}
			files = append(files, f)
