...
import (
	...
	"github.com/BlackCodes/micro-swagger/protoc-gen-micro-swagger/internal/httprule"
	openapi_options "github.com/grpc-ecosystem/grpc-gateway/v2/protoc-gen-micro-swagger/options"
...
func TestSchemaOfField(t *testing.T) {
	...
	var microOptions = new(descriptorpb.microOptions)
	proto.SetExtension(microOptions, openapi_options.E_Openapiv2Field, jsonSchema)
	...
	tests := []test{
		...
		},
		{
			...
			openAPIOptions: &openapiconfig.OpenAPIOptions{
				Field: []*openapiconfig.OpenAPImicroOption{
					{
...
func TestRenderMessagesAsDefinition(t *testing.T) {
	...
	tests := []struct {
		...
		{
			descr: "external optionss option",
			msgDescs: []*descriptorpb.DescriptorProto{
				{Name: proto.String("Message")},
			},
			schema: map[string]openapi_options.Schema{
				"Message": {
					Externaloptionss: &openapi_options.Externaloptionsumentation{
						Description: "glorious optionss",
						Url:         "https://nada",
					},
				},
			},
			defs: map[string]openapiSchemaObject{
				"Message": {
					...
					Externaloptionss: &openapiExternaloptionsumentationObject{
						Description: "glorious optionss",
						URL:         "https://nada",
					},
				},
...
func TestUpdateOpenAPIDataFromComments(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedOpenAPIObject: &openapiSchemaObject{
				Title:       "First line",
				Description: "{{import \"optionsumentation.md\"}}",
			},
			comments:      "First line\n\n{{import \"optionsumentation.md\"}}",
			expectedError: nil,
		},
		{
