...
import (
	...
	"github.com/BlackCodes/micro-swagger/protoc-gen-micro-swagger/internal/descriptor/openapiconfig"
...
type Registry struct {
	...
	// microOptions is a mapping of the fully-qualified name of the parent message concat
	// field name and a period to additional OpenAPI field options
	microOptions map[string]*options.JSONSchema
	...
	// omitPackageoptions, if false, causes a package comment to be included in the generated code.
	omitPackageoptions bool

	// optionsument server show options
	optionsServer string

	// server port
	optionsServerPort int64

	// interface prefix path
	apiPrefix string

	// project of proto name
	project string
}
...
func NewRegistry() *Registry {
	return &Registry{
		...
		microOptions:   make(map[string]*options.JSONSchema),
	}
}
...
// SetOmitPackageoptions controls whether the generated code contains a package comment (if set to false, it will contain one)
func (r *Registry) SetOmitPackageoptions(omit bool) {
	r.omitPackageoptions = omit
}

// GetOmitPackageoptions returns whether a package comment will be omitted from the generated code
func (r *Registry) GetOmitPackageoptions() bool {
	return r.omitPackageoptions
}

// SetoptionsServer set options server address and check is normal.
func (r *Registry) SetoptionsServer(server string) {
	r.optionsServer = server
}

// SetoptionsServer set options server address and check is normal.
func (r *Registry) SetoptionsServerPort(port int64) {
	if port < 80 || port > 65535 {
		panic("the port excess max port")
	}
	r.optionsServerPort = port
}

// SetProject set project name
func (r *Registry) SetProject(project string) {
	r.project = project
}
...
// GetoptionsServer returns a options server ,if null return nil
func (r *Registry) GetoptionsServer() string {
	return fmt.Sprintf("%s:%d", r.optionsServer, r.optionsServerPort)
}
...
func (r *Registry) RegisterOpenAPIOptions(opts *openapiconfig.OpenAPIOptions) error {
	...
	for _, opt := range opts.Field {
		...
		r.microOptions[qualifiedField] = opt.Option
	}
	return nil
}
...
// GetOpenAPImicroOption returns a registered OpenAPI option for a field
func (r *Registry) GetOpenAPImicroOption(qualifiedField string) (*options.JSONSchema, bool) {
	opt, ok := r.microOptions[qualifiedField]
	return opt, ok
}
